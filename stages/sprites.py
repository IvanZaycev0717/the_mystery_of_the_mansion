...
class Generic(pygame.sprite.Sprite):
    """Parent class for tiles and objects."""
...
class Player(Generic):
    """Class for player and for his behavior."""
    def __init__(
            self,
            pos,
            assets,
    ...
    def input(self):
        ...
        if keys[pygame.K_RIGHT] and not self.is_sitting and self.status != 'death':
            ...
            if self.status != 'jump' and self.status != 'fall':
                self.walk_sound.set_volume(
                    float(self.config['SOUNDS']['sounds']))
        ...
        if keys[pygame.K_SPACE] and self.on_floor and self.status != 'death':
            ...
            self.direction.y = -2
            self.jump_sound.set_volume(float(self.config['SOUNDS']['sounds']))
    ...
    def check_on_floor(self):
        ...
        floor_sptites = [
            sprite for sprite in self.collision_sprites if sprite.rect.colliderect(floor_rect)]
...
class Animated(Generic):
    """Class for animated tiles and objects."""
    def __init__(self, assets, pos, group, z=LEVEL_LAYERS['main']):
        self.animation_frames = assets
        self.frame_index = 0
        super().__init__(
            pos, self.animation_frames[self.frame_index], group, z)
...
class Taken(Animated):
    """Class for gears when you have taken the one."""
    def __init__(self, assets, pos, group):
        super().__init__(assets, pos, group)
        self.rect = self.image.get_rect(center=pos)
...
class Spikes(Generic):
    """Class for enemies such as spikes."""
...
class Fire(Animated):
    """Class for enemies such as a fire."""
    def __init__(self, assets, pos, group):
        super().__init__(assets, pos, group)


class Slime(Animated):
    """Class for enemies such as slime."""
    def __init__(self, assets, pos, group):
        super().__init__(assets, pos, group)


class Camel(Generic):
    """Class for enemies such as a camel."""
    ...
    def animate(self, dt):
        if not self.has_shot:
            ...
            Splutter(self.rect.center + vector(-60, -30), splutter_direction,
                     self.splutter_surf, [self.groups(), self.enemy_sprites])
            self.has_shot = True
...
class Splutter(Generic):
    """Class for camel's splutter when it has spat on the player."""
...
class Wasp(Generic):
    """Class for enemies such as a wasp."""
    ...
    def move(self, dt):
        ...
        if [sprite for sprite in self.collision_sprites if sprite.rect.collidepoint(
...
class FlyingEnemy(Generic):
    """Class for enemies such as a bat, scroll."""
...
class WalkingEnemies(Generic):
    """Class for enemies such as a bug, a hedgehog."""
    def __init__(self, assets, pos, group, collision_sprites):
        self.animation_frames = assets
        self.frame_index = 0
        surf = self.animation_frames[self.frame_index]
    ...
    def move(self, dt):
        if not [
            sprite for sprite in self.collision_sprites if sprite.rect.collidepoint(
                self.rect.midbottom + vector(0, 10))]:
            self.kill()
        self.pos.x -= self.speed * dt
        self.rect.x = round(self.pos.x)
...
class Angel(Generic):
    """Class for enemies such as an angel."""
    def __init__(self, assets, pos, group, collision_sprites):
        #  general setup
        self.animation_frames = assets
        self.frame_index = 0
        ...
        if not [
            sprite for sprite in collision_sprites if sprite.rect.collidepoint(
                self.rect.midbottom + vector(0, 10))]:
            self.kill()
...
class Goat(Generic):
    """Class for enemies such as a goat."""
    def __init__(self, assets, pos, group, collision_sprites):
        self.animation_frames = assets
        self.frame_index = 0
        self.orientation = 'right'
...
class Harp(Generic):
    """Class for enemies such as a harp."""
    ...
    def animate(self, dt):
        ...
        if not self.has_shot:
            ...
            Arrow(self.rect.center + vector(0, -30), arrow_direction,
                  self.arrow_surf, [self.groups(), self.enemies_sprites])
            self.has_shot = True
...
class Arrow(Generic):
    """Class for enemies such as an arrow from a harp."""
    def __init__(self, pos, direction, surf, group):
        super().__init__(pos, surf, group)
        self.mask = pygame.mask.from_surface(self.image)
...
class Bird(Generic):
    """Class for enemies such as a bird."""
    def __init__(self, assets, pos, group, collision_sprites):
        self.animation_frames = assets
        self.frame_index = 0
        self.orientation = 'right'
