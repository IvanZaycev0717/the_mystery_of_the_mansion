...
class UI:
    """Class for showing amount of lives and health of the player."""
    ...
    def show_hp(self, current, full):
        ...
        health_bar_rect = pygame.Rect(
            self.hp_bar_topleft, (current_bar_width, self.bar_height))
        pygame.draw.rect(
...
class Inventory:
    """Class for showing what items the player has."""
    def __init__(self, surface, config):
        ...
        self.green_key_img = load(
            'images/ui/inventory/green_key.png').convert_alpha()
        self.green_key_rect = self.green_key_img.get_rect(x=997, y=140)
        self.hammer_img = load(
            'images/ui/inventory/hammer.png').convert_alpha()
        self.hammer_rect = self.hammer_img.get_rect(x=997, y=200)
        self.pink_key_img = load(
            'images/ui/inventory/pink_key.png').convert_alpha()
        self.pink_key_rect = self.pink_key_img.get_rect(x=1000, y=270)
        self.yellow_key_img = load(
            'images/ui/inventory/yellow_key.png').convert_alpha()
    ...
    def show_inventory(self, gears_amount, player_stats):
        ...
        self.inventory_insc = self.font.render(
            eng.INV_INSC if self.lang == 'eng' else rus.INV_INSC, False, 'yellow')
        self.inventory_insc_rect = self.inventory_insc.get_rect(x=1025, y=30)

        # Images of items
        self.display_surface.blit(
            self.inventory_insc,
            self.inventory_insc_rect)
        ...
        # Status of items
        self.gears_amount = self.font.render(
            f'x{gears_amount}', False, 'yellow')
        ...
        self.green_status = self.font.render(
            (eng.KEY_FOUND if player_stats['green_key'] else eng.KEY_NOT_FOUND) if self.lang == 'eng' else (
                rus.KEY_FOUND if player_stats['green_key'] else rus.KEY_NOT_FOUND), False, 'yellow')
        ...
        self.hammer_status = self.font.render(
            (eng.KEY_FOUND if player_stats['hammer_key'] else eng.KEY_NOT_FOUND) if self.lang == 'eng' else (
                rus.KEY_FOUND if player_stats['hammer_key'] else rus.KEY_NOT_FOUND), False, 'yellow')
        ...
        self.pink_status = self.font.render(
            (eng.KEY_FOUND if player_stats['pink_key'] else eng.KEY_NOT_FOUND) if self.lang == 'eng' else (
                rus.KEY_FOUND if player_stats['pink_key'] else rus.KEY_NOT_FOUND), False, 'yellow')
        ...
        self.yellow_status = self.font.render(
            (eng.KEY_FOUND if player_stats['yellow_key'] else eng.KEY_NOT_FOUND) if self.lang == 'eng' else (
                rus.KEY_FOUND if player_stats['yellow_key'] else rus.KEY_NOT_FOUND), False, 'yellow')
...
class Helper:
    """Class for showing hints to the player."""
    def __init__(self, surface, config):
        # setup
        self.display_surface = surface
        self.config = config
    ...
    def show_helper(self, message):
        ...
        self.message = self.font.render(
            eng.HLP[message] if self.lang == 'eng' else rus.HLP[message], False, 'yellow')
        self.message_rect = self.message.get_rect(
            centerx=WINDOW_WIDTH / 2, centery=37)
