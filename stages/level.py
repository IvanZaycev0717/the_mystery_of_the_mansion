...
class Level:
    """Parent Level class for building levels."""
    def __init__(
            self,
            grid,
            switch,
        ...
        self.level_limits = {'left': -WINDOW_WIDTH,
                             'right': sorted(list(grid['common'].keys()),
                                             key=lambda pos: pos[0])[-1][0] + 500}
            ...
            for pos, data in layer.items():
                if layer_name == 'common':
                    if isinstance(data, tuple) and isinstance(data[1], int):
                        Generic(pos, asset_dict['land'][data[0]][data[1]], [
    ...
    def get_keys(self):
        collided_keys = pygame.sprite.spritecollide(
            self.player, self.keys_sprites, True)
    ...
    def get_damage(self):
        collision_sprites = pygame.sprite.spritecollide(
            self.player, self.enemies_sprites, False, pygame.sprite.collide_mask)
    ...
    def get_gears(self):
        collided_gears = pygame.sprite.spritecollide(
            self.player, self.gear_sprites, True)
...
class CameraGroup(pygame.sprite.Group):
    """Performs camera movement depending on player's events."""
    ...
    def draw_horizon(self):
        ...
        if horizon_pos < WINDOW_HEIGHT:
            ...
            pygame.draw.rect(
                ...
                HORIZON_TOP_COLOR,
                horizon_rect3)
            pygame.draw.line(self.dispay_surface, HORIZON_COLOR,
                             (0, horizon_pos), (WINDOW_WIDTH, horizon_pos), 3)
...
class Common(Level):
    """Class for common level, outside of mansion."""
    def __init__(
            self,
            grid,
            switch,
    ...
    def get_activator(self, dt=0):
        collision_sprites = pygame.sprite.spritecollide(
            self.player, self.activator_sprites, False)
        self.gate_active = False
        self.door_in = False
        if collision_sprites:
            for sprite in collision_sprites:
                if sprite.__dict__[
                        'id'] == 'blue_door_in' and not self.player_stats['yellow_key']:
                    self.helper.show_helper('yellow_not')
                if sprite.__dict__[
                        'id'] == 'blue_door_in' and self.player_stats['yellow_key']:
    ...
    def event_loop(self, dt):
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN and event.key in (
                    pygame.K_x, pygame.K_UP) and self.gate_active:
                self.transition()
                self.bg_music.stop()
            ...
            if event.type == pygame.KEYDOWN and event.key in (
                    pygame.K_x, pygame.K_UP) and self.door_in:
                self.transition()
                self.bg_music.stop()
...
class Cementry(Level):
    """Class for cementry level, outside of mansion."""
    def __init__(
            self,
            grid,
            switch,
...
class Hall(Level):
    """Class for hall level, inside of mansion."""
    def __init__(
            self,
            grid,
            switch,
            ...
            has_clouds=True,
            has_horizon=True):
        super().__init__(grid, switch, asset_dict, audio, gear_change, hp, change_keys,
                         sky_color, ground_color, set_prev_stage, config, has_clouds, has_horizon)
        self.current_stage = 6
        self.prev_stage = 3
        self.door_out = False
    ...
    def get_activator(self):
        ...
        if collision_sprites:
            for sprite in collision_sprites:
                if sprite.__dict__[
                        'id'] == 'blue_door_out' and self.player_stats['yellow_key']:
                    self.helper.show_helper('has_item')
                    self.door_out = True
                ...
                if sprite.__dict__[
                        'id'] == 'green_door_out' and self.player_stats['green_key']:
                    self.helper.show_helper('has_item')
                ...
                if sprite.__dict__[
                        'id'] == 'pink_door_out' and self.player_stats['pink_key']:
                    self.helper.show_helper('has_item')
                    self.pink_door = True
    ...
    def event_loop(self):
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYUP and event.key == pygame.K_TAB:
                self.tab_pressed = False
            if event.type == pygame.KEYDOWN and event.key in (
                    pygame.K_x, pygame.K_UP) and self.door_out:
                self.transition()
                self.bg_music.stop()
                self.current_stage = 4
                self.door_out = False
            ...
            if event.type == pygame.KEYDOWN and event.key in (
                    pygame.K_x, pygame.K_UP) and self.cupboard_door:
                self.transition()
                self.bg_music.stop()
            ...
            if event.type == pygame.KEYDOWN and event.key in (
                    pygame.K_x, pygame.K_UP) and self.green_door:
                self.transition()
                self.bg_music.stop()
            ...
            if event.type == pygame.KEYDOWN and event.key in (
                    pygame.K_x, pygame.K_UP) and self.pink_door:
                self.transition()
                self.bg_music.stop()
...
class Cupboard(Level):
    """Class for cupboard level, inside of mansion."""
    def __init__(
            self,
            grid,
            switch,
        ...
        if collision_sprites:
            for sprite in collision_sprites:
                if sprite.__dict__['id'] == 'cup_door_out':
                    ...
                    self.cupboard_door = True
                if sprite.__dict__[
                        'id'] == 'cup_bed' and not self.player_stats['green_key']:
    ...
    def event_loop(self):
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN and event.key in (
                    pygame.K_x, pygame.K_UP) and self.cupboard_bed:
                self.transition()
                self.bg_music.stop()
...
class Heaven(Level):
    """Class for heaven level, in the player's dream."""
    def __init__(
            self,
            grid,
            switch,
...
class FirstFloor(Level):
    """Class for first floor level, inside the mansion."""
    def __init__(
            self,
            grid,
            switch,
    ...
    def get_activator(self):
        ...
        if collision_sprites:
            for sprite in collision_sprites:
                if sprite.__dict__['id'] in (
                        'green_door_in', 'green_door_out'):
                    self.helper.show_helper('has_item')
                    self.green_door = True
                if sprite.__dict__[
                        'id'] == 'ff_bed_in' and not self.player_stats['pink_key']:
                    self.helper.show_helper('sleep_well')
                    self.green_bed = True
    ...
    def event_loop(self):
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN and event.key in (
                    pygame.K_x, pygame.K_UP) and self.green_bed:
                self.transition()
                self.bg_music.stop()
...
class Desert(Level):
    """Class for desert level, in the player's dream."""
    def __init__(
            self,
            grid,
            switch,
...
class SecondFloor(Level):
    """Class for second floor level, inside the mansion."""
    def __init__(
            self,
            grid,
            switch,
        ...
        if collision_sprites:
            for sprite in collision_sprites:
                if sprite.__dict__['id'] == 'pink_door_in':
                    ...
                    self.pink_door = True
                if sprite.__dict__[
                        'id'] == 'sf_bed' and not self.player_stats['hammer_key']:
                    self.helper.show_helper('sleep_well')
                    self.pink_bed = True
                if sprite.__dict__[
                        'id'] == 'act_wall' and not self.player_stats['hammer_key']:
                    self.helper.show_helper('hammer_not')
                if sprite.__dict__[
                        'id'] == 'act_wall' and self.player_stats['hammer_key']:
    ...
    def event_loop(self):
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN and event.key in (
                    pygame.K_x, pygame.K_UP) and self.pink_bed:
                self.transition()
                self.bg_music.stop()
            ...
            if event.type == pygame.KEYDOWN and event.key in (
                    pygame.K_x, pygame.K_UP) and self.wall:
                self.transition()
                self.bg_music.stop()
...
class Garden(Level):
    """Class for garden level, in the player's dream."""
    def __init__(
            self,
            grid,
            switch,
...
class Poison(Level):
    """Class for poison level, inside the mansion."""
    def __init__(
            self,
            grid,
            switch,
    ...
    def event_loop(self):
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYUP and event.key == pygame.K_TAB:
                self.tab_pressed = False
            if event.type == pygame.KEYDOWN and event.key in (
                    pygame.K_x, pygame.K_UP) and self.machine:
                self.bg_music.stop()
                self.machine = False
                self.is_music_playing = False
