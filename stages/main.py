...
class Main:
    ...
    def imports(self):
        ...
        # STATIC
        self.cementry_stuff = {
            folder: import_folder(f'images/objects/cementry/{folder}') for folder in (
                list(
                    os.walk('images/objects/cementry/')))[0][1]}
        self.common_stuff = {folder: import_folder(
            f'images/objects/common/{folder}') for folder in (list(os.walk('images/objects/common/')))[0][1]}
        self.desert_stuff = {folder: import_folder(
            f'images/objects/desert/{folder}') for folder in (list(os.walk('images/objects/desert/')))[0][1]}
        self.cupboard_stuff = {
            folder: import_folder(f'images/objects/cupboard/{folder}') for folder in (
                list(
                    os.walk('images/objects/cupboard/')))[0][1]}
        self.first_floor_stuff = {
            folder: import_folder(f'images/objects/first_floor/{folder}') for folder in (
                list(
                    os.walk('images/objects/first_floor/')))[0][1]}
        self.floor_stuff = {folder: import_folder(
            f'images/objects/floor/{folder}') for folder in (list(os.walk('images/objects/floor/')))[0][1]}
        self.garden_stuff = {folder: import_folder(
            f'images/objects/garden/{folder}') for folder in (list(os.walk('images/objects/garden/')))[0][1]}
        self.heaven_stuff = {folder: import_folder(
            f'images/objects/heaven/{folder}') for folder in (list(os.walk('images/objects/heaven/')))[0][1]}
        self.poison_stuff = {folder: import_folder(
            f'images/objects/poison/{folder}') for folder in (list(os.walk('images/objects/poison/')))[0][1]}

        # ACTIVATORS
        self.activators = {
            folder: import_folder(f'images/objects/activators/{folder}') for folder in (
                list(
                    os.walk('images/objects/activators/')))[0][1]}

        # ENEMIES
        # cementry enemies
        self.bat = import_folder('images/enemies/cementry/bat/')
        self.bug = import_folder('images/enemies/cementry/bug/')
        self.cem_spikes = load(
            'images/enemies/cementry/spikes/1.png').convert_alpha()

        # desert enemies
        self.camel = load('images/enemies/desert/camel/1.png').convert_alpha()
        self.fire = import_folder('images/enemies/desert/fire/')
        self.bird = {folder: import_folder(f'images/enemies/desert/fire_bird/{folder}')
                     for folder in (list(os.walk('images/enemies/desert/fire_bird/')))[0][1]}
        self.goat = {folder: import_folder(f'images/enemies/desert/goat/{folder}')
                     for folder in (list(os.walk('images/enemies/desert/goat/')))[0][1]}
        self.splutter = load(
            'images/enemies/desert/splutter/1.png').convert_alpha()

        # garden enemies
        self.hedgehog = import_folder('images/enemies/garden/hedgehog/')
        self.gar_spikes = load(
            'images/enemies/garden/spikes/1.png').convert_alpha()
        self.wasp = load('images/enemies/garden/wasp/1.png').convert_alpha()

        # heaven enemies
        self.angel = {
            folder: import_folder(f'images/enemies/heaven/angel/{folder}') for folder in (
                list(
                    os.walk('images/enemies/heaven/angel/')))[0][1]}
        self.harp = import_folder('images/enemies/heaven/harp/')
        self.scrolls = import_folder('images/enemies/heaven/scrolls/')
        self.heav_spikes = load(
            'images/enemies/heaven/spikes/1.png').convert_alpha()
        ...
        # player
        self.player_graphics = {folder: import_folder(
            f'images/player/{folder}') for folder in (list(os.walk('images/player/')))[0][1]}
    ...
    def run(self):
        while True:
            ...
            self.fullscreen_mode = self.config.getboolean(
            ...
            match self.stage:
                case 0:
                    if self.editor_active:
                        ...
                        self.level.run(
                            dt, self.gears, self.player_stats, self.stage)
                ...
                case 2:
                    if self.config.get('LANG', 'Lang') not in ('eng', 'rus'):
                        ...
                        self.stage = 3
                case 3:
                    self.stage = self.main_menu.run(
                        dt, self.stage, self.prev_stage)
                    self.transition.display(dt)
                case 4:
                    self.stage = self.common_level.run(
                        dt, self.gears, self.player_stats, self.stage, self.prev_stage)
                    self.ui.show_lives(self.player_stats['lives'])
                    self.ui.show_hp(
                        self.player_stats['current_hp'],
                        self.player_stats['max_hp'])
                    self.common_transmission(self.common_level, dt)
                    self.transition.display(dt)
                case 5:
                    self.stage = self.cementry_level.run(
                        dt, self.gears, self.player_stats, self.stage)
                    self.ui.show_lives(self.player_stats['lives'])
                    self.ui.show_hp(
                        self.player_stats['current_hp'],
                    ...
                    self.transition.display(dt)
                case 6:
                    self.stage = self.hall_level.run(
                        dt, self.gears, self.player_stats, self.stage)
                    self.ui.show_lives(self.player_stats['lives'])
                    self.ui.show_hp(
                        self.player_stats['current_hp'],
                    ...
                    self.transition.display(dt)
                case 7:
                    self.stage = self.cupboard_level.run(
                        dt, self.gears, self.player_stats, self.stage)
                    self.ui.show_lives(self.player_stats['lives'])
                    self.ui.show_hp(
                        self.player_stats['current_hp'],
                    ...
                    self.transition.display(dt)
                case 8:
                    self.stage = self.heaven_level.run(
                        dt, self.gears, self.player_stats, self.stage)
                    self.ui.show_lives(self.player_stats['lives'])
                    self.ui.show_hp(
                        self.player_stats['current_hp'],
                    ...
                    self.transition.display(dt)
                case 9:
                    self.stage = self.first_floor_level.run(
                        dt, self.gears, self.player_stats, self.stage)
                    self.ui.show_lives(self.player_stats['lives'])
                    self.ui.show_hp(
                        self.player_stats['current_hp'],
                    ...
                    self.transition.display(dt)
                case 10:
                    self.stage = self.desert_level.run(
                        dt, self.gears, self.player_stats, self.stage)
                    self.ui.show_lives(self.player_stats['lives'])
                    self.ui.show_hp(
                        self.player_stats['current_hp'],
                    ...
                    self.transition.display(dt)
                case 11:
                    self.stage = self.second_floor_level.run(
                        dt, self.gears, self.player_stats, self.stage)
                    self.ui.show_lives(self.player_stats['lives'])
                    self.ui.show_hp(
                        self.player_stats['current_hp'],
                    ...
                    self.transition.display(dt)
                case 12:
                    self.stage = self.garden_level.run(
                        dt, self.gears, self.player_stats, self.stage)
                    self.ui.show_lives(self.player_stats['lives'])
                    self.ui.show_hp(
                        self.player_stats['current_hp'],
                    ...
                    self.transition.display(dt)
                case 13:
                    self.stage = self.poison_level.run(
                        dt, self.gears, self.player_stats, self.stage)
                    self.ui.show_lives(self.player_stats['lives'])
                    self.ui.show_hp(
                        self.player_stats['current_hp'],
                    ...
                    self.transition.display(dt)
                case 'GAME_OVER':
                    self.stage = self.game_over.run(
                        dt, self.config.get('LANG', 'Lang'))
                    self.transition.display(dt)
                case 'END':
                    self.stage = self.end.run(
                        dt, self.config.get('LANG', 'Lang'))
                    self.transition.display(dt)
                case 'CS1':
                    self.stage = self.cs1.run(
                        dt, self.config.get('LANG', 'Lang'))
                    self.transition.display(dt)
                case 'CS2':
                    self.stage = self.cs2.run(
                        dt, self.config.get('LANG', 'Lang'))
                    self.transition.display(dt)
                case 'CS3':
                    self.stage = self.cs3.run(
                        dt, self.config.get('LANG', 'Lang'))
                    self.transition.display(dt)
                case 'CS4':
                    self.stage = self.cs4.run(
                        dt, self.config.get('LANG', 'Lang'))
                    self.transition.display(dt)
                case 'CS5':
                    self.stage = self.cs5.run(
                        dt, self.config.get('LANG', 'Lang'))
                    self.transition.display(dt)
                case 'CS6':
                    self.stage = self.cs6.run(
                        dt, self.config.get('LANG', 'Lang'))
                    self.transition.display(dt)
                case 'BE':
                    self.stage = self.bad_end.run(
                        dt, self.config.get('LANG', 'Lang'))
                    self.transition.display(dt)
                case 'GE':
                    self.stage = self.good_end.run(
                        dt, self.config.get('LANG', 'Lang'))
                    self.transition.display(dt)
...
class Transition:
    """Class for transitions between different stages."""
