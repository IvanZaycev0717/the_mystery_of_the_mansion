...
class Editor:
    """Runs the game as the editor's mode."""
    ...
    def get_current_cell(self, obj=None):
        distance_to_origin = (
            vector(mouse_pos()) - self.origin if not obj
    ...
    def canvas_add(self):
        if (mouse_buttons()[0] and not self.menu.rect.collidepoint(
                mouse_pos()) and not self.object_drag_active):
                ...
                if current_cell != self.last_selected_cell:
                    ...
                    if current_cell in self.canvas_data:
                        ...
                        self.canvas_data[current_cell] = CanvasTile(
                            self.selection_index, self.selection_terrain
                        )
    ...
    def canvas_remove(self):
        if mouse_buttons()[2] and not self.menu.rect.collidepoint(mouse_pos()):
            ...
            if self.canvas_data:
                current_cell = self.get_current_cell()
                if current_cell in self.canvas_data:
                    self.canvas_data[current_cell].remove_id(
                        self.selection_index
                    )
    ...
    def draw_tile_lines(self):
        ...
        for col in range(cols + 1):
            ...
            pygame.draw.line(self.support_line_surf,
                             LINE_COLOR, (x, 0), (x, WINDOW_HEIGHT))

        for row in range(rows + 1):
            y = origin_offset.y + row * TILE_SIZE
            pygame.draw.line(self.support_line_surf,
                             LINE_COLOR, (0, y), (WINDOW_WIDTH, y))
    ...
    def draw_level(self):
        ...
        for cell_pos, tile in self.canvas_data.items():
            ...
            if tile.has_terrain and isinstance(
                    tile.has_terrain,
                    int) and isinstance(
                    tile.terrain_id,
                    int):
    ...
    def create_grid(self):
        ...
        for obj in self.canvas_objects:
            ...
            offset = vector(obj.distance_to_origin) - \
                (vector(current_cell) * TILE_SIZE)
        ...
        for tile_pos, tile in self.canvas_data.items():
            ...
            if tile.objects:
                for obj, offset in tile.objects:
                    if obj in [
                            key for key,
                            value in EDITOR_DATA.items() if value['style'] == 'activator']:
                        layers['activators'][(
                            int(x + offset.x), int(y + offset.y))] = obj
                    elif obj in [key for key, value in EDITOR_DATA.items() if value['style'] == 'static']:
                        layers['static'][(int(x + offset.x),
                                          int(y + offset.y))] = obj
                    else:
                        layers['fg objects'][int(
                            x + offset.x), int(y + offset.y)] = obj
        return layers

    # sky and clouds section
    ...
    # events handler and running editors mode
    def event_loop(self):
...
class CanvasTile:
    """Class for the tiles such as terrain, enemies, gears and keys."""
...
class CanvasObjects(pygame.sprite.Sprite):
    """Class for the objects such as player, static and active objects."""
    ...
    def animate(self, dt):
        ...
        self.frame_index = 0 if self.frame_index >= len(
            self.frames) else self.frame_index
